...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    const imageContainer = document.getElementById('custom_image_preview_container');
    
    imageContainer.addEventListener('click', function (e) {
        if (e.target && e.target.closest('.custom-remove-button')) {
            const imageDiv = e.target.closest('.custom-image-container');
            const imageId = imageDiv.getAttribute('data-image-id');
            
            // Ẩn ảnh
            imageDiv.style.display = 'none';
            
            // Tạo input ẩn chứa ID ảnh bị xóa
            const hiddenInput = document.createElement('input');
            hiddenInput.type = 'hidden';
            hiddenInput.name = 'deleted_images[]';
            hiddenInput.value = imageId;
            
            // Thêm input ẩn vào container
